        ...
        {activeSection === 'roadmap' && (
          <div className="animate-in fade-in duration-700">
            ...
            <div className="space-y-12">
              {[
                {
                  phase: 'Phase 1: Multi-Agent Choreography',
                  desc: 'Moving from single prompts to "Agent Handshakes" where specialized bots (Security, Perf, QA) collaborate autonomously.',
                  status: 'Upcoming'
                },
                {
                  phase: 'Phase 2: Temporal Multimodal Debugging',
                  desc: 'Models (Veo 3.1) watching UI sessions and syncing them to backend log trace IDs automatically.',
                  status: 'In Lab'
                },
                {
                  phase: 'Phase 3: Local SLMs (Small Language Models)',
                  desc: 'Running 8B models (Gemini Nano 2) on local dev machines for real-time, private code auditing.',
                  status: 'Foundational'
                }
              ].map((item, index) => (
                <div 
                  key={index} 
                  className="p-12 bg-white border border-zinc-100 rounded-[3.5rem] relative group overflow-hidden"
                >
